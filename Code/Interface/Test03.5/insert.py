...
def make_put_request(patient_id, first_name, middle_name, last_name1, last_name2, birth_date, dni, allergies, create_file):
    url = f"http://localhost:52773/csp/test0305/insertpath/{patient_id}/{first_name}/{middle_name}/{last_name1}/{last_name2}/{birth_date}/{dni}/{allergies}/{create_file}"
...
def main():
    ...
    #
    if st.button("Submit"):
        result = make_put_request(patient_id, first_name, middle_name, last_name1, last_name2, birth_date, dni, allergies, create_file)
        #
        if "error" in result:
